...
class RecommendationEngine:
    ...
    def score_candidate(self, candidate: Dict[str, Any], learner_state: Dict[str, Any], skill_gap: Dict[str, Any]) -> float:
        ...
        coverage_score = overlap / max(1, len(gap_skills)) if gap_skills else 0
    ...
    def get_hybrid_recommendations(self, candidates: List[Dict[str, Any]], learner_profile: Dict[str, Any], skill_gaps: List[str] = None) -> List[Dict[str, Any]]:
        """
        Score and rank candidate resources based on skill gaps and learner profile dynamically.
        """
        if not candidates:
            return []
            
        skill_gap_dict = {"missing_skills": skill_gaps or []}
        
        scored_candidates = []
        for candidate in candidates:
            score = self.score_candidate(candidate, learner_profile, skill_gap_dict)
...
# Keep the original function signature for backwards compatibility but use the real engine
def get_hybrid_recommendations(skill_gaps: List[str], learner_profile: Dict[str, Any], candidates: List[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    if candidates is None:
        return []
    return recommendation_engine.get_hybrid_recommendations(candidates, learner_profile, skill_gaps)
